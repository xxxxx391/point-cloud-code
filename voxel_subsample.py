...
def read_and_downsample_ply(file_path, voxel_size=1):
    """
...
def scale_point_cloud(pcd, output_file_path):
    """
    ...
    try:
        ...
        print(f"点云坐标已成功缩小100倍并保留三位小数，保存到 {output_file_path}")
    except Exception as e:
        print(f"处理过程中出现错误: {e}")

if __name__ == "__main__":
    # 替换为你的 PLY 文件路径
    ply_file_path = "xixihe_TLS - Cloud - Cloud.ply"
    # 调用函数进行读取和下采样
    downsampled_cloud = read_and_downsample_ply(ply_file_path)
